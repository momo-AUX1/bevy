...
impl AssetReader for FileAssetReader {
    ...
    async fn read_directory<'a>(
        ...
        let full_path = self.root_path.join(path);
        fn map_dir(read_dir: std::fs::ReadDir, root_path: PathBuf) -> Result<Box<PathStream>, AssetReaderError> {
            ...
            Ok(Box::new(DirReader(mapped_stream.collect())))
        }

        match read_dir(&full_path) {
            Ok(read_dir) => map_dir(read_dir, self.root_path.clone()),
            Err(e) => {
                if e.kind() == std::io::ErrorKind::NotFound {
                    // On WinRT/UWP, prefer LocalState but fall back to packaged assets.
                    #[cfg(all(target_os = "windows", __WINRT__))]
                    if let Some(fallback_root) = self.fallback_root_path() {
                        ...
                        match read_dir(&fallback_full_path) {
                            Ok(read_dir) => return map_dir(read_dir, fallback_root.clone()),
                            Err(e) => {
                                if e.kind() != std::io::ErrorKind::NotFound {
                                    return Err(e.into());
                                }
